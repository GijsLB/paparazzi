...
#define Y_BLACK 10  // Consecutive black pixels before turning white pixels to black
#define X_WHITE 8  // Consecutive white pixels before turning black pixels to white
...
void filter_ground(uint8_t *image_data, int width, int height, int *horizon) {
    for (int x = 0; x < width; x++) {
        ...
        horizon[x] = (highest_white == -1) ? height : highest_white;
...
void process_image(const char *input_filename, const char *output_filename) {
    ...
    int *horizon = malloc(width * sizeof(int));
